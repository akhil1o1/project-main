...
export const createJoke = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Could not create the joke, please try again later.", 500)
    );
  }
...
export const updateJoke = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Could not update the joke, please try again later.", 500)
    );
  }
...
export const deleteJoke = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Could not delete the joke, please try again later.", 500)
    );
  }

  if (!user) {
    return next(new HttpError("Authentication failed!!! you are not allowed to delete this joke.", 404));
  }
  ...
  if (!userIsAdmin && joke.creator.id !== req.userId) { //if user is neither admin nor creator
    return next(
      new HttpError(
